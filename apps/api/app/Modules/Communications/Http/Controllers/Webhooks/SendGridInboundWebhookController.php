...
{
    public function __invoke(Request $request, CommunicationCommandService $commandService, CommunicationAttachmentService $attachmentService, CommunicationAuditService $auditService): JsonResponse
    {
        ...
        $commandService->appendEvent((string) $client->tenant_id, (string) $client->id, 'communication_message', (string) $message->id, 'sendgrid.inbound.received', 'received', (string) $message->correlation_key, true, $request->all(), 'sendgrid', null, null, null, 'received');
        $auditService->record(null, (string) $client->tenant_id, 'communication.email.inbound_received', 'communication_message', (string) $message->id, (string) Str::uuid(), ['fromAddress' => $fromAddress]);
