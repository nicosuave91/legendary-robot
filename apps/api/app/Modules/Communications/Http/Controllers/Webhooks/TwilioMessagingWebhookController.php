...
final class TwilioMessagingWebhookController extends Controller
{
    public function __invoke(Request $request, CommunicationCommandService $commandService, CommunicationAuditService $auditService, CommunicationsWebhookTrustService $trustService): JsonResponse
    {
        ...
        $commandService->appendEvent((string) $message->tenant_id, (string) $message->client_id, 'communication_message', (string) $message->id, 'twilio.messaging.callback', $providerStatus, (string) $message->correlation_key, $trust->verified, $request->all(), 'twilio', (string) ($request->input('SmsSid') ?: $message->provider_message_id), (string) Str::uuid(), $statusBefore, $nextStatus);
        $auditService->record(null, (string) $message->tenant_id, 'communication.sms.callback_processed', 'communication_message', (string) $message->id, (string) Str::uuid(), ['providerStatus' => $providerStatus, 'statusAfter' => $nextStatus, 'signatureVerified' => $trust->verified, 'webhookTrustMode' => $trust->mode]);
