  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      notify({
        title: 'Draft saved',
        description:
          'The mutable draft version remains separate from published rule history.',
        tone: 'success',
      })
    },
    onError: (error) =>
  ...
  const publishMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      notify({
        ...
        description: 'The latest rule version is now immutable and execution-ready.',
        tone: 'success',
      })
    },
    onError: (error) =>
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <div className="text-xs text-text-muted">
                          Subject {log.subjectType} #{log.subjectId} • version {log.ruleVersionId}
                        </div>
                      </div>
                      <AppBadge
                        variant={
                          log.outcome === 'blocking' || log.outcome === 'error'
                            ? 'danger'
                            : log.outcome === 'applied'
                              ? 'success'
                              : 'info'
        ...
        <div className="space-y-5">
          <AppCard>
            ...
            <AppCardBody className="space-y-3 body-sm text-text-muted">
              ...
              <p>
                Current status:{' '}
                <span className="font-medium text-text">{payload.rule.status}</span>
              </p>
              <p>
                Latest published version:{' '}
                <span className="font-medium text-text">
                  v{payload.rule.latestPublishedVersionNumber ?? '—'}
                </span>
              </p>
              <p>
                Current draft version:{' '}
                <span className="font-medium text-text">
                  v{payload.rule.currentDraftVersionNumber ?? '—'}
                </span>
              </p>
            </AppCardBody>
          </AppCard>
