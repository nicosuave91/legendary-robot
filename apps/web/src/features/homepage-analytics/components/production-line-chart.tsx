    ...
    <AppCard>
      <AppCardHeader>
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="body-sm text-text-muted">Activity over the last {window.replace('d', ' days')}.</div>
            <div className="flex flex-wrap gap-4">
              {data.series.map((series, index) => (
                <div key={series.key} className="inline-flex items-center gap-2 text-xs text-text-muted">
                  <span className="inline-block h-0.5 w-4 rounded-full" style={{ backgroundColor: chartColors[index] }} />
      ...
      <AppCardBody>
        {!hasData ? (
          <EmptyState title="No production movement yet" description="Activity will appear here once client, note, or document work begins." />
        ) : (
          <svg viewBox="0 0 640 132" className="h-32 w-full overflow-visible rounded-lg border border-border bg-muted/30 p-3" role="img" aria-label="Production trend chart">
