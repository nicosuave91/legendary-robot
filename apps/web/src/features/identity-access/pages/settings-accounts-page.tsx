...
export function SettingsAccountsPage() {
  ...
  const createAccountMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      notify({
        ...
        description: 'The new user now defaults to onboarding required on first sign-in.',
        tone: 'success'
      })
    }
  })
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Accounts"
        description="Tenant-scoped user provisioning with default onboarding enforcement for admin-created users."
      />
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)]">
        <AppCard>
          <AppCardHeader>
            ...
            <div className="body-sm text-text-muted">
              Current users, roles, and onboarding state come from the server-authoritative settings API.
            </div>
          </AppCardHeader>
          <AppCardBody>
            <div className="space-y-3">
              {accounts.map((account) => (
                <div key={account.id} className="rounded-lg border border-border bg-muted p-4">
                  ...
                  <div className="body-sm mt-2 text-text-muted">Roles: {account.roles.join(', ')}</div>
                  <div className="body-sm text-text-muted">Onboarding: {account.onboardingState}</div>
                </div>
              ))}
            </div>
          </AppCardBody>
        ...
        <AppCard>
          <AppCardHeader>
            ...
            <div className="body-sm text-text-muted">
              Keep this narrow for Sprint 2: create user, assign role, enforce onboarding.
            </div>
          </AppCardHeader>
          <AppCardBody>
