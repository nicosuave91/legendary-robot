...
import { AppButton, AppCard, AppCardBody, AppCardHeader, AppInput, AppSelect, AppTextarea, EmptyState, LoadingSkeleton, PageHeader } from '@/components/ui'
...
const defaultConditionDefinition = JSON.stringify({ all: [{ fact: 'amountRequested', operator: 'gte', value: 250000 }] }, null, 2)
const defaultActionDefinition = JSON.stringify({ type: 'application_note', outcome: 'warning', title: 'High-value review note', bodyTemplate: 'This application matched a governed review rule and should receive additional human review.' }, null, 2)
...
export function RulesListPage() {
  ...
  const createMutation = useMutation({
    mutationFn: async () => rulesApi.create({
    ...
    onSuccess: async (response) => {
      ...
      notify({ title: 'Draft rule created', description: 'The rule catalog now includes an editable Sprint 8 draft version.', tone: 'success' })
      navigate(`/app/rules/${ruleId}`)
    },
    onError: (error) => {
      notify({ title: 'Rule draft failed', description: error instanceof Error ? error.message : 'The draft could not be created.', tone: 'danger' })
    }
  })
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Rules Library"
        description="Published rules stay immutable. Drafts remain editable until publish, and Applications now consume governed rule versions instead of a hard-coded bridge."
      />
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.3fr)_420px]">
        <AppCard>
          <AppCardHeader>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <div className="body-sm text-text-muted">Filter by lifecycle or module scope and open a rule to inspect versions and execution evidence.</div>
              </div>
          ...
          <AppCardBody>
            ...
            {!listQuery.isLoading && items.length === 0 ? (
              <EmptyState title="No rules yet" description="Create the first governed draft rule to establish an immutable publication history." />
            ) : null}
            <div className="space-y-3">
              {items.map((item) => (
                <Link key={item.id} to={`/app/rules/${item.id}`} className="block rounded-lg border border-border bg-muted p-4 transition hover:border-primary/40 hover:bg-surface">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <div className="heading-md text-text">{item.name}</div>
                      <div className="body-sm text-text-muted">{item.ruleKey} • {item.moduleScope} • latest published v{item.latestPublishedVersionNumber ?? '—'}</div>
                    </div>
                  ...
                  <div className="mt-3 grid gap-3 text-xs text-text-muted sm:grid-cols-3">
                    ...
                    <div>Published at: {item.latestPublishedAt ? new Date(item.latestPublishedAt).toLocaleString() : 'Not published'}</div>
        ...
        <AppCard>
          <AppCardHeader>
            ...
            <div className="body-sm text-text-muted">Draft creation stores a stable rule identity plus an editable version 1 definition.</div>
          </AppCardHeader>
          <AppCardBody>
            <div className="space-y-4">
              <div className="space-y-2"><label className="label-sm text-text">Rule key</label><AppInput value={form.ruleKey} onChange={(event) => setForm((current) => ({ ...current, ruleKey: event.currentTarget.value }))} placeholder="app-high-value-review" /></div>
              <div className="space-y-2"><label className="label-sm text-text">Name</label><AppInput value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.currentTarget.value }))} placeholder="High-value application review" /></div>
              <div className="space-y-2"><label className="label-sm text-text">Description</label><AppTextarea value={form.description} onChange={(event) => setForm((current) => ({ ...current, description: event.currentTarget.value }))} placeholder="Explain where this rule applies in the funnel." /></div>
              ...
              <AppButton type="button" onClick={() => createMutation.mutate()} disabled={createMutation.isPending || !form.ruleKey.trim() || !form.name.trim()}>
