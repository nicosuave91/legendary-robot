...
final readonly class SubmitOutboundSmsJob implements ShouldQueue, QueuesTenantAware
{
    ...
    public function handle(SmsTransportProvider $provider, CommunicationCommandService $communicationCommandService, CommunicationAuditService $auditService): void
    {
        ...
        try {
            ...
            $auditService->record(null, (string) $message->tenant_id, $result->accepted ? 'communication.sms.submitted' : 'communication.sms.submit_failed', 'communication_message', (string) $message->id, $this->correlationIdValue, $result->toAuditSummary());
            ...
            $communicationCommandService->appendEvent((string) $message->tenant_id, (string) $message->client_id, 'communication_message', (string) $message->id, 'provider_submission_exception', 'failed', (string) $message->correlation_key, false, ['message' => $throwable->getMessage()], 'twilio', null, null, $statusBefore, 'failed');
            $auditService->record(null, (string) $message->tenant_id, 'communication.sms.submit_failed', 'communication_message', (string) $message->id, $this->correlationIdValue, ['message' => $throwable->getMessage()]);
        }
    }
