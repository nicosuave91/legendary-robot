...
const items = [
  ...
  { to: '/app/calendar', label: 'Calendar', icon: CalendarDays, permissions: ['calendar.read'] as const },
  { to: '/app/communications', label: 'Communications', icon: MessageSquare, permissions: ['clients.communications.read'] as const },
...
export function AppSidebar({ collapsed, onToggle }: AppSidebarProps) {
  const { data } = useAuth()
  const visibleItems = items.filter((item) => hasAllPermissions(data?.permissions ?? [], [...item.permissions]))
    ...
    >
      <div className="mb-6 flex items-start justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          {!collapsed ? <AppBadge variant="info">Closure phase</AppBadge> : null}
        </div>
      </div>
