...
type WorkflowRuntimeClientSubjectType = (typeof WORKFLOW_RUNTIME_CLIENT_SUBJECT_TYPES)[number]
...
export function validateBuilderStateBeforeSave(state: WorkflowBuilderState): string[] {
  const errors: string[] = []
  const normalizedSubjectType = state.trigger.subjectType.trim()
  ...
  state.trigger.filters.forEach((filter, index) => {
    if (!filter.fact.trim()) errors.push(`Trigger filter ${index + 1}: fact is required.`)
    if (filter.operator !== 'exists' && !filter.value.trim()) {
      errors.push(`Trigger filter ${index + 1}: value is required.`)
    }
  })
  ...
  state.steps.forEach((step, index) => {
    ...
    if (
      ['create_client_note', 'send_sms', 'send_email'].includes(step.type) &&
      normalizedSubjectType &&
      !WORKFLOW_RUNTIME_CLIENT_SUBJECT_TYPES.includes(normalizedSubjectType as WorkflowRuntimeClientSubjectType)
    ) {
      errors.push(
        `Step ${index + 1}: ${WORKFLOW_STEP_TYPE_LABELS[step.type]} currently supports only ${WORKFLOW_RUNTIME_CLIENT_SUBJECT_TYPES.join(' or ')} subject types.`
      )
    }
...
export const WORKFLOW_TEMPLATE_PRESETS: WorkflowTemplatePreset[] = [
  ...
  },
  {
    ...
    description: 'Send an SMS after an application event using a runtime-safe application subject.',
      ...
      steps: [
        {
          ...
          type: 'send_sms',
          definition: {
            bodyTemplate: 'We received your application and will follow up with next steps shortly.'
          }
        }
      ]
    }
